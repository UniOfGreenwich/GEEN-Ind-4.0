...
# This one allows you to move the coloured blocks at the top around by pinching and dragging them.
# You can select to draw circles or lines by pinching and opening on the controls at the bottom.
# P = Pick Mode (No drawing)
# L = Line Mode Pinch / drag to draw a line.
# C = Circle Mode Pinch / Drag from center point to set radius of circle.
...
def drawControls(index):
    ...
    for ob in controlList:
        ...
        cv2.putText(controls, str(ob[TEXT]), (ob[XCOORD] + 20, ob[YCOORD] + 80), font, 3, (255, 255, 255), 2,
                    cv2.LINE_AA)
        if ptr == index:
            cv2.rectangle(controls, (ob[XCOORD], ob[YCOORD]), (ob[XCOORD] + RECTSIZEX, ob[YCOORD] + RECTSIZEY),
                          (255, 255, 0), 1)
...
controlList = [[50,350,200,0,0,10,"P"], [160,350,200,0,0,10,"C"], [270,350,200,0,0,10,""],[380,350,200,0,0,10,"L"],[490,350,200,0,0,10,""]]

objectList = [[100,50,0,200,0,1,"1"], [210,50,200,0,0,1,"2"], [320,50,0,0,200,1,"3"]]
...
while True:
    ...
    if results.multi_hand_landmarks:
        ...
        if rPick and not rGrab:                    # We have a pick but nothing grabbed
            ...
            # Let's check the controls in case we have pinched on a control.
            controlNumber = controlFound(xCoord,yCoord)
            if  controlNumber!= -1:
                # We need to pick it up and start moving it.
                rGrab = False   # Set the flag
                rPick = False
                cPinch = False
                drawControls(controlNumber)
                print("Control found = ",controlNumber)
                controlSelected = controlNumber
